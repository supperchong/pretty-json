...
export function activate(context: vscode.ExtensionContext) {
    ...
    let disposable = vscode.commands.registerTextEditorCommand('extension.prettyJson', (textEditor: TextEditor, edit: TextEditorEdit, args: any[]) => {
        // The code you place here will be executed every time your command is executed
        let selectText=textEditor.document.getText(textEditor.selections[0]);
        let prettyText:string;
        try{
            prettyText=pretty(selectText,{delComment:true});
        }catch(err){
